...
figma.ui.onmessage = async (msg: ConvertMsg | IngestMsg | ChatSseMsg | ContextSseMsg | GenerateMsg | GenerateLlmMsg) => {
  if (msg.type === 'convert') {
    try {
      ...
      figma.ui.postMessage({ type: 'result', ok: false, error: String((e as Error && (e as Error).message) || e) });
    }
    return;
  }
  ...
  if (msg.type === 'ingest') {
    ...
    try {
      ...
      const ingRes = await retry(() => fetch(edge + '/figma-compat/uui/ingest', { method: 'POST', headers: headerObj, body: JSON.stringify({ projectId: msg.payload.projectId, storage_key: storageKey }) }), { retries: 2, backoffMs: 700 });
      ...
      for (let i = 0; i < 20; i++) {
        ...
        if (r.ok && j && j.ok && j.status === 'ready') { ready = true; outKey = (j && j.key) || ''; outSigned = (j && j.signedUrl) || null; break; }
      }
  ...
  if (msg.type === 'chat_sse') {
    try {
      ...
      figma.ui.postMessage({ type: 'sse_error', source: 'chat', error: String((e as Error && (e as Error).message) || e) });
    }
    return;
  }
  ...
  if (msg.type === 'context_sse') {
    try {
      ...
      figma.ui.postMessage({ type: 'sse_error', source: 'figma-context', error: String((e as Error && (e as Error).message) || e) });
    }
    return;
  }
